...
#include <linux/soc/owl/hardware.h>
#include <linux/soc/owl/irqs.h>
#include <linux/soc/owl/hdmac-owl.h>
#include <linux/soc/owl/bootdev.h>
...
//CEM_CHANGE extern int acts_wlan_set_power(struct wlan_plat_data *pdata, int on,unsigned long msec);
//CEM_CHANGE extern struct wlan_plat_data *acts_get_wlan_plat_data(void);
...
//CEM_CHANGE 	struct wlan_plat_data *pdata;
	int ret = 0;
...
static int acts_mmc_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	if (mmc&&(mmc->card)&&(mmc->card->type == MMC_TYPE_SDIO)){
		printk(KERN_INFO "CEM DEBUG something wrong not suppose execute this %s:%d\n", __func__, __LINE__);
#if 0//CEM_CHANGE 
