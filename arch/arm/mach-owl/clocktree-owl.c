...
static struct clk_init_data *owl_clks_init;
...
{
	if (clk->core->parent)
		return clk->core->parent->new_rate;
...
{
	clk_calc_subtree(clk->core, newrate, NULL, 0);
}
...
int owl_clk_config_recursion(int clock, int recursion)
{
	...
	if (clock >= 0 && clock <= CLOCK__MAX) {
		ops = owl_clks_init[clock].ops;
		if (ops == &clk_ops_direct_s_parent || ops == &clk_ops_direct_m_parent) {
			if (recursion) {
				owl_clks_init[clock].flags |= CLK_SET_RATE_PARENT;
			} else {
				owl_clks_init[clock].flags &=  ~CLK_SET_RATE_PARENT;
			}
			return 0;
		...
		ops = clkops_select_version(owl_clks_init[clock].ops, recursion);
		if (ops == NULL) {
			return -1;
		}
		if (ops == owl_clks_init[clock].ops) {
			return 0;
		}
		if (recursion == 1) {
			owl_clks_init[clock].flags |= CLK_SET_RATE_PARENT;
...
static void __init init_clocktree(void)
{
	...
	if (of_machine_is_compatible("actions,atm7059a")) {
        ...
        owl_clks_init = clks_table->owl_clks_init;
...
static int owl_limit_notify(struct notifier_block *nb, unsigned long action, void *data)
{
	...
	if (action == PRE_RATE_CHANGE) {
		cnd = data;
		foo = to_clk_foo(cnd->clk->core->hw);
		clock = foo->clock;
...
int __init owl_init_clocks(void)
{
	...
	spin_lock_init(&cpu_lock);
   	...
   	struct clk *owl_clk, *owl_clk1; 
	char *tmp, tmp1[32];
	owl_clk = kmalloc(sizeof(struct clk), GFP_KERNEL);
	owl_clk1 = kzalloc(sizeof(struct clk), GFP_KERNEL);
	tmp = kmalloc(sizeof(char)*32, GFP_KERNEL);
	if(!owl_clk)
		printk("CEM DEBUG %s:%d\n", __func__, __LINE__);
	if(!owl_clk1)
		printk("CEM DEBUG %s:%d\n", __func__, __LINE__);
	strcpy(tmp1,"testingcpy1");
	printk("CEM DEBUG %s:%d %s\n", __func__, __LINE__,tmp1);
	strcpy(tmp,"testingcpy");
	...
	for (i = 0; i < CLOCK__MAX + MOD__MAX_IN_CLK; i++) {
	    if(owl_clks_init[i].ops){
    		//__clk_init(NULL, &owl_clks[i]);
		
		printk("CEM DEBUG %s:%d i:%d name:%s\n", __func__, __LINE__, i, owl_clks_init[i].name);
    		owl_clks[i] = clk_register(NULL, lookup_table[i].clk_hw);
		lookup_table[i].clk = owl_clks[i];
	    }
	}
#endif
	prepare_clocktree();
	...
	for (i = 0; i < CLOCK__MAX; i++) {
		calcfrequency(i);
		clk_round_rate(owl_clks[i], clocks[i].frequency);
	}
